# Contrato operacional único — revisão 2026-09-10
...
Merge pertence ao revisor técnico, após CI verde da base atual e revisão. Usar merge commit para preservar SHA auditável. Nunca force-push, unrelated-histories, ignorar CI ou sobrescrever worktree como recuperação genérica. Concluir card somente após provar integração remota. Main não é destino nesta PoC.

Para aprovar/integrar, o worker REVISOR salva sua análise independente fora do worktree (por exemplo `/tmp/review-ID.md`) e executa `/opt/hermes/.venv/bin/python /opt/hermes/review_merge.py --task ID --pr NUMERO --sha SHA --evidence /tmp/review-ID.md`. O controlador verifica identidade/run, matriz, PR/SHA/branch, limpeza e CI, registra análise, publica `hermes-independent-review` e pede merge ao GitHub com o SHA exato. Sem esse check o GitHub bloqueia merge. Se falhar, corrija causa ou peça mudanças; não emita status manualmente.
...
Impedimento: categoria, causa, comandos/logs, condição objetiva de retomada. Supervisor cria INCIDENT independente do DAG; especialista→Tech Lead→CTO. Sem recursão de INCIDENT/RECOVERY. CTO executa hipótese/experimento limitado com especialista; não repete retry sem mudança de evidência. Dois reclaims sem progresso estacionam para diagnóstico. Heartbeat não é entrega.
